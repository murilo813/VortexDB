...
def update_header_field(PageHeader, buffer, updates):
    if "free_start" in updates or "free_end" in updates:
        new_free_start = updates.get(
            "free_start", PageHeader["free_start"]
        )  # é feito essa variacao aqui pra caso nao venha 1 dos dois, assim o código não trava por não ter o campo no dict
...
def iter_records(buffer):
    ...
    while offset < PAGE_SIZE:
        ...
        record_size = int.from_bytes(
            buffer[offset + 1 : offset + 3], byteorder="little"
        )
